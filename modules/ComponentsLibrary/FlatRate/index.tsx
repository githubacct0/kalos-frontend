...
import { makeFakeRows, QuoteLineClientService, TimesheetDepartmentClientService, usd, UserClientService } from '../../../helpers';
...
export const FlatRateSheet: React.FC<Props> = function FlatRateSheet({
  ...
  const handleTogglePendingDelete = (selectedRow: number, remove: boolean) => {
    dispatch({ type: 'setPendingDelete', data: {
  ...
  const getUserData = useCallback(async() => {
    ...
    const canEditFlatRate = user.getPermissionGroupsList().find(p=>p.getName() === 'EditFlatRates');
    const canEditDepartments = user.getPermissionGroupsList().find(p=>p.getName() === 'EditFlatRateDepartment');
  ...
  const handleSave = async(id: number, data?: any) => {
    ...
    if (id === 0) {
      ...
      flatRate.setAdjustment(state.pendingSave.cost.replace("$", "").replace(" ", ""));
  ...
  const getFlatRates = useCallback(async (refresh: boolean) => {
    ...
    if (refresh) {
      ...
      filteredQLResults = qlResults.filter(rate => {
        if (state.searchParams.description !== '' && rate.getDescription().search(new RegExp(state.searchParams.description, "i")) === -1) {
          return false;
        }
        if (state.searchParams.cost !== '' && rate.getAdjustment().search(new RegExp(state.searchParams.cost)) === -1 ) {
          return false;
        }
        if (state.searchParams.department.length > 0) {
          let foundDepartments = state.searchParams.department.filter(dep => rate.getDepartments().split(",").includes(String(dep)));
      ...
      dispatch({ type: 'setData', data: { flatRates: qlResults, filteredFlatRates: filteredQLResults, loaded: true } });
    } else {
  ...
  const getDepartments = useCallback(async() => {
    ...
    try {
      ...
      dispatch({ type: 'setDepartmentList', data: departmentList.getResultsList() });
    } catch (err) {
  ...
  return (
    ...
    />
    {/* @ts-ignore*/}
    <Box sx={{width:'100%', position: 'sticky', top:{xs:'35px', md:'48px'}, zIndex:3, backgroundColor: "white", borderTop:"0px", borderLeft:"0px", borderRight:"0px", border: "solid", borderColor:"black"}}>
      <div>
        <PlainForm
      ...
      {state.loaded && (
        <InfoTable
          ...
          rowButton={{
            ...
            columnDefinition: {
              ...
              columnTypeOverrides: [
                ...
                { columnName: 'Department', columnType: 'multiselect'
                , options: state.departmentList.map(dl => ({
                  label: `${dl.getValue()} - ${dl.getDescription()}`,
                  value: dl.getId(),
                })),
          ...
          columns={[
            ...
            { name: 'Actions', width: window.innerWidth * 0.1, invisible: !state.canEditFlatRates },
          ]}
            ...
            return [
              {
                value:
                  state.remove && state.selectedRowId === value.getId() ? (
                    <Typography style={{textAlign: "center", fontWeight:"bolder", fontSize:"20px"}}>
              ...
              },
              {
                value:
                  state.remove && state.selectedRowId === value.getId() ? (
                    <div></div>
                  ) : state.edit && state.selectedRowId === value.getId() && state.canEditDepartment ? (
                    <PlainForm
                      onChange={result => {
                        let pendingSave = state.pendingSave;
                    ...
                    'All Departments' :
                    state.departmentList.filter(dep=>value.getDepartments().split(",").includes(String(dep.getId()))).map(dep=>dep.getValue()).toString()
                  ),
              },
              {
                value: (
                  <div>
                    {state.selectedRowId !== value.getId() && state.canEditFlatRates && (
                      <div>
                        <Tooltip title="Edit">
                          <IconButton
                            onClick={() =>
                              handleToggleEdit('edit', value.getId(), {
                                ...
                                department: value.getDepartments().split(",").map(dep=>Number(dep)),
                              })
                            }
                          >
                            <EditIcon />
                        ...
                        <Tooltip title="Delete">
                          <IconButton onClick={() => handleTogglePendingDelete(value.getId(), true)}>
                            <DeleteIcon />
                          </IconButton>
                        </Tooltip>
                      </div>
                    )}
                    {state.edit && state.selectedRowId === value.getId() && state.canEditFlatRates && (
                      <div>
                        <Tooltip title="Save">
                    ...
                    {state.remove && state.selectedRowId === value.getId() && state.canEditFlatRates && (
                      <div>
