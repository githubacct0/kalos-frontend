...
const initialFormData: FormData = {
  ...
  availableTechs: [],
  meetingTime: format(
    addDays(setMinutes(setHours(new Date(), 7), 30), 1),
    'yyyy-MM-dd HH:mm',
  ),
  classTime: format(
    addDays(setMinutes(setHours(new Date(), 7), 30), 1),
...
const initialFirstCall: FirstCallType = {
  ...
  inUse: [],
  expires: format(
    addDays(setSeconds(setMinutes(setHours(new Date(), 0), 0), 0), 1),
    'yyyy-MM-dd HH:mm:ss',
  ),
  message: '',
  onCall: {
...
export const FirstCallDashboard: React.FC<Props> = function FirstCallDashboard({
  ...
  const handleNotification = useCallback(
    (message: string | string[], type: string, hasError = true) => {
      let updateNotification = true;
      let errorMessage = state.notificationMessage;
      if (typeof message === 'string') {
        if (!errorMessage.includes(message)) {
          if (type === state.notificationType) {
            // If error/warning is same as current error/warning, then stack.
            errorMessage = errorMessage.concat(message);
          } else if (state.notificationType === 'error' && hasError === true) {
            // Prevent warning message from overwriting error message.
  ...
  const checkErrors = useCallback(
    (errorMessage: string) => {
      if (
        ...
        state.notificationMessage.includes(errorMessage)
      ) {
        ...
        if (state.notificationMessage.length > 1) {
          ...
          newMessage.splice(
            newMessage.findIndex(msg => msg === errorMessage),
            1,
          );
  ...
  const resetModal = async (refreshCalls = false, save = false) => {
    switch (state.modalKey) {
      case 'Class': {
        ...
        state.classTechs.map(tech =>
          techList.push({ id: tech.getUserId(), name: tech.getTechname() }),
        );
        const data = {
        ...
        if (
          data.start !== state.firstCallClass.start ||
          !equals(data.list, state.firstCallClass.list)
        ) {
      ...
      case 'Meeting': {
        ...
        if (
          data.start !== state.firstCallMeeting.start ||
          !equals(data.list, state.firstCallMeeting.list)
        ) {
      ...
      case 'callInfo': {
        ...
        if (save) {
          ...
          for (const i in state.assigneeList) {
            const tech = state.techs.find(
              tech => tech.getUserId() === state.assigneeList[i].id,
            )!;
            newAssignees.push({
          ...
          const fcCallIndex = state.firstCallCalls.findIndex(
            call => call.id === currentFCCall.id,
          );
          if (fcCallIndex >= 0 && currentFCCall.assigned.length)
            // Replace Current Entry if there are Assignees
            firstCalls[fcCallIndex] = currentFCCall;
          else if (fcCallIndex >= 0)
            // Remove Current Entry if there are no Assignees
            firstCalls.splice(
              firstCalls.findIndex(
                call => call.id === state.selectedCall.getId(),
              ),
              1,
            );
          else if (currentFCCall.assigned.length)
            // Add New Entry if Assignees
          ...
          for (const j in state.pendingRemoveInUse) {
            fcInUse.splice(
              fcInUse.findIndex(tech => tech === state.pendingRemoveInUse[j]),
              1,
            );
          }
          updateFirstCallState({
    ...
    if (refreshCalls) {
      // If Edit Service Call Modal is Saved
  ...
  const handleFormDataUpdate = useCallback(
    async (data: any, timesOnly = false) => {
      let resetPage = false;
      ...
      if (data.division && data.division !== state.formData.division) {
        ...
        try {
          ...
          updateFormData.departmentIds = departments
            .getResultsList()
            .map(dep => dep.getId());
        } catch (err) {
          console.error(err);
  ...
  const handleCallDetails = async (
    ...
    if (editCall) {
      ...
      let currentFCCall = state.firstCallCalls.find(
        fcCall => fcCall.id === call.getId(),
      );
      let errorMessage = '';
      ...
      if (!currentFCCall) {
        for (const i in callAssignees) {
          if (callAssignees[i] != '' && callAssignees[i] != '0') {
            const tech = state.techs.find(
              tech => tech.getUserId() === Number(callAssignees[i]),
            );
            if (tech) {
        ...
        if (techsAlreadyAssigned.length) {
          ...
          for (const i in techsAlreadyAssigned) {
            if (i === '0') {
              ...
              i === String(techsAlreadyAssigned.length - 1) &&
              techsAlreadyAssigned.length === 2
            ) {
        ...
        if (techsNotFound.length) {
          errorMessage += `- Could Not Add `;
          const usersNotFound = state.users.filter(user =>
            techsNotFound.includes(user.getId()),
          );
          for (const j in usersNotFound) {
            if (j === '0') {
              errorMessage += `${usersNotFound[
                j
              ].getFirstname()} ${usersNotFound[j].getLastname()}`;
            } else if (
              j === String(usersNotFound.length - 1) &&
              usersNotFound.length === 2
            ) {
              errorMessage += ` and ${usersNotFound[
                j
              ].getFirstname()} ${usersNotFound[j].getLastname()}`;
      ...
      for (const i in currentFCCall.assigned) {
        assigneeList.push({
          id: currentFCCall.assigned[i].userId,
          name: currentFCCall.assigned[i].name,
        });
      }
      updateFirstCallState({
        type: 'setModal',
        data: {
          ...
          selectedCall: call,
          currentFC: state.savedFirstCall,
          callMsg: state.firstCallCalls.find(
            fcCall => fcCall.id === call.getId(),
          )
            ? state.firstCallCalls.find(fcCall => fcCall.id === call.getId())!
                .notes
            : call.getNotes(),
        },
      });
  ...
  const getAvailableTechs = useCallback(
    (
      ...
      const manualOffTechs = manualOff.length
        ? manualOff
        : state.firstCallManualOff;
      const inUseTechs = inUse.length ? inUse : state.firstCallInUse;
      const scheduledOffTechs = scheduledOff.length
        ? scheduledOff
        : state.firstCallScheduledOff;
  ...
  const handleUpdateAssignTech = async (id: number, updateType: string) => {
    ...
    if (updateType === 'remove') {
      ...
      if (addInUse.includes(id)) {
        addInUse.splice(
          addInUse.findIndex(tech => tech === id),
          1,
        );
      }
      currentAssignees.splice(
        currentAssignees.findIndex(tech => tech.id === id),
        1,
      );
    } else {
      addInUse.push(id);
      if (removeInUse.includes(id)) {
        removeInUse.splice(
          removeInUse.findIndex(tech => tech === id),
          1,
        );
      }
  ...
  const handleUnOffTech = async (tech: DispatchableTech) => {
    ...
    const index = offTechs.findIndex(
      offTech => offTech.id === tech.getUserId(),
    );
    ...
    if (center.lat === 0 && center.lng === 0) {
      if (address) {
        ...
        try {
          ...
          console.error(`An error occurred while geocoding: ${err}`);
          handleNotification(
        ...
        handleNotification(
          '- No Valid Latitude, Longitude, or Address found.',
          'error',
        );
      }
    }
      ...
      for (const j in state.firstCallCalls[i].assigned) {
        ...
        try {
          ...
          errorMessage += `- Failed to Assign ${
            state.firstCallCalls[i].assigned[j].name
          } to Call ${state.selectedCall.getId()}`;
        }
      }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (!allowSave) {
        updateFirstCallState({
          type: 'setFailedSave',
          data: { save: false, error: 'DateMismatch' },
        });
        handleNotification(
          '- A Newer Version Exists.  Please Refresh the Page.',
          'warning',
        );
      } else {
        if (state.newFirstCall) {
      ...
      updateFirstCallState({
        type: 'setFailedSave',
        data: { save: false, error: 'Error' },
      });
      handleNotification('- Failed to Save First Call Information.', 'error');
    }
    updateFirstCallState({ type: 'setProcessing', data: false });
  }, [
    state.firstCallCalls,
    state.firstCallMeeting,
    state.firstCallClass,
    state.firstCallManualOff,
    state.firstCallInUse,
    state.firstCallOnCall,
    state.firstCallMessage,
    state.firstCallExpires,
    state.firstCallId,
    ...
    if (state.firstCallCalls.length) {
      ...
      for (let call in state.firstCallCalls) {
        for (let user in state.firstCallCalls[call].assigned) {
          if (testUserId > 0) {
            SlackClientService.Dispatch(
              state.firstCallCalls[call].id,
              testUserId,
              loggedUserId,
              true,
            );
            ...
            SlackClientService.Dispatch(
              state.firstCallCalls[call].id,
              state.firstCallCalls[call].assigned[user].userId,
              loggedUserId,
              true,
            );
          }
  ...
  const handleGeocodeUpdate = async (
    ...
    if (type === 'user') {
      ...
      users.forEach(user => {
        if (user.getId() === id) {
          user.setGeolocationLat(geo.lat);
          user.setGeolocationLng(geo.lng);
        }
      });
      ...
      calls.forEach(call => {
        if (call.getPropertyId() === id) {
          call.setGeolocationLat(geo.lat);
          call.setGeolocationLng(geo.lng);
        }
      });
      ...
      for (let i in sortedFirstCalls) {
        const fcCall = calls
          .slice(0)
          .find(call => call.getId() === sortedFirstCalls[i].id);
        const logTechAssigned: string[] = [];
        const logTechNames: string[] = [];
        if (fcCall) {
          sortedFirstCalls[i].assigned.forEach(tech => {
            logTechAssigned.push(String(tech.userId));
            logTechNames.push(tech.name);
          });
  ...
  const getTechs = useCallback(
    async (userList: User[] = state.users) => {
      ...
      const offRequestTimeStarted = format(
        addDays(setHours(setMinutes(setSeconds(new Date(), 0), 0), 7), 1),
        'yyyy-MM-dd HH::mm::ss',
      );
      const offRequestTimeFinished = format(
        addDays(setHours(setMinutes(setSeconds(new Date(), 0), 0), 12), 1),
        'yyyy-MM-dd HH::mm::ss',
      );
      off.setIsActive(1);
      off.setRequestStatus(1);
      off.setDateRangeList([
        '>=',
        offRequestTimeStarted,
        '<=',
        offRequestTimeFinished,
      ]);
      off.setDateTargetList(['time_finished', 'time_started']);
      off.setDepartmentIdList(state.formData.departmentIds.toString());
      try {
        const userTechList = userList.filter(
          user =>
            user.getIsHvacTech() === 1 &&
            state.formData.departmentIds.includes(
              user.getEmployeeDepartmentId(),
            ),
        );
        const dispatchableTechs =
          await DispatchClientService.GetDispatchableTechnicians(techs);
        const techList = dispatchableTechs.getResultsList();
        const techIds = techList.map(tech => tech.getUserId());
        const missingTechs = userTechList.filter(
          user => !techIds.includes(user.getId()),
        );
        ...
        techList
          .filter(tech =>
            timeoffList
              ...
              .includes(tech.getUserId()),
          )
          .forEach(tech =>
            timeoffIds.push({ id: tech.getUserId(), name: tech.getTechname() }),
          );
  ...
  const getCalls = useCallback(async () => {
    ...
    try {
      ...
      if (state.formData.division === 1) {
        ...
        requestedCallList = callList
          .getResultsList()
          .filter(call => call.getLogJobstatus() === `Requested`);
        filteredCallList = callList.getResultsList().filter(call => {
          return (
            call.getDateStarted() <=
              format(addDays(new Date(), 1), 'yyyy-MM-dd') &&
            call.getDateEnded() >=
  ...
  const setCalls = useCallback(
    async (skipGet = false, firstCalls?: CallObj[]) => {
  ...
  const getGoogleApiKey = useCallback(async () => {
    ...
    try {
      ...
      console.error(`An error occurred while getting Google API Key: ${err}`);
  ...
  const getPreviousFirstCall = useCallback(async () => {
    ...
    try {
      ...
      return {
        ...
        newFirstCall: false,
        firstCallId: previousFirstCall.getId(),
        saveTime: previousFirstCall.getDateCreated(),
        error: '',
      };
    } catch (err) {
      return {
        firstCall: initialFirstCall,
        newFirstCall: true,
        firstCallId: 0,
        saveTime: '',
        error: '',
  ...
  const checkDivision = useCallback(async () => {
    ...
    try {
      ...
      const userDepartments = userData
        .getPermissionGroupsList()
        .filter(user => user.getType() === 'department')
        .reduce(
      ...
      let filteredDepartments = departments
        .getResultsList()
        .filter(dep => userDepartments.includes(dep.getId()));
      if (!filteredDepartments.length) {
        filteredDepartments = departments
          .getResultsList()
          .filter(dep => dep.getId() === userData.getEmployeeDepartmentId());
      }
      const sectorList = filteredDepartments.map(sector =>
        sector.getSectorGroup(),
      );
      const filteredSectorList = sectorList
        .filter((c, index) => sectorList.indexOf(c) === index && c !== 0)
    ...
    try {
      if (url.searchParams.get('code')) {
        ...
        const token = await axios.get(
          `https://slack.com/api/oauth.access?client_id=${slackClientId}&client_secret=${secretKey.getApiKey()}&code=${url.searchParams.get(
            'code',
          )}&redirect_uri=https://app.kalosflorida.com/index.cfm?action=admin:dispatch.firstcall`,
        );
  ...
  const getInitialConstructor = useCallback(async () => {
    ...
    const newFormData: FormData = {
      ...
      availableTechs: getAvailableTechs(
        results[0].techList,
        results[4].firstCall.manualOff,
        results[4].firstCall.inUse,
        results[0].scheduledOff,
        true,
      )!,
    };
    ...
    [
      ...
      },
      {
        ...
        options: [
          ...
          { name: 'Commercial Light', value: 2 },
          { name: 'Commercial Heavy', value: 3 },
        ]
  ...
  useEffect(() => {
    ...
    if (
      !state.loaded &&
      state.formData.division !== 0 &&
      state.formData.departmentIds.length !== 0
    ) {
      load();
    }
  ...
  return (
    <PageWrapper userID={loggedUserId}>
          ...
          >
            ...
            {state.notificationMessage.map(message =>
              message
                .split('     ')
                .map((msg, index) => (
                  <Typography key={`${index}_notifications`}>{msg}</Typography>
                )),
            )}
            {state.notificationType === 'error' && (
      ...
      {!state.userHasApiKey && state.checkUser && (
        ...
        >
          ...
          >
            ...
            >
              You need to register your Slack account to the Dispatch App before
              you can use First Calls.
            </Typography>
            <Typography
              style={{
                textAlign: 'center',
            ...
            >
              Please click the following link to register. In the dropdown,
              enter and select the dispatch channel for your department and then
              click Authorize.
            </Typography>
            <div
              style={{
      ...
      {!state.loaded && state.formData.division !== 0 && (
        <Loader backgroundColor={'black'} opacity={0.5} />
      )}
      {state.loaded && (
        <Grid container spacing={1} style={{ paddingTop: '20px' }}>
          <Grid item xs={12}>
              ...
              if (callback.destination) {
                switch (callback.destination.droppableId) {
                  case 'onCallDroppable': {
                    const selectedTech = state.techs.find(
                      tech => tech.getUserId() === Number(callback.draggableId),
                    );
                    if (selectedTech) {
                      const newOnCall = {
                      ...
                      if (
                        newOnCall.tech.userId !==
                        state.firstCallOnCall.tech.userId
                      ) {
                        updateFirstCallState({
                  ...
                  case 'dismissTech': {
                    ...
                    manualOffTechs = manualOffTechs.concat({
                      id: selectedTech!.getUserId(),
                      name: selectedTech!.getTechname(),
                    });
                    getAvailableTechs([], manualOffTechs, [], [], false);
                    updateFirstCallState({
                      type: 'setFirstCallManualOff',
                      data: manualOffTechs,
                    });
                    break;
                  }
                  default: {
                    ...
                    const selectedCall = state.calls.filter(
                      call =>
                        call.getId() ===
                        Number(callback.destination?.droppableId),
                    )[0];
                    const techAssign = state.techs.find(
                      tech => tech.getUserId() === Number(callback.draggableId),
                    )!;
          ...
          >
            ...
            <Grid
              item
              xs={12}
              style={{ margin: 'auto', display: 'flex', paddingTop: '20px' }}
            >
              <ButtonGroup
                fullWidth
                variant="text"
              ...
              >
                ...
                >
                  {state.meetingTechs.length
                    ? state.formData.meetingTime.substring(11, 12) === '0' &&
                      state.formData.meetingTime.substring(12, 13) !== '0'
                      ? `Meeting Tomorrow @ ${state.formData.meetingTime.substring(
                          12,
                        )}`
                      : `Meeting Tomorrow @ ${state.formData.meetingTime.substring(
                          11,
                ...
                >
                  {state.classTechs.length
                    ? state.formData.classTime.substring(11, 12) === '0' &&
                      state.formData.meetingTime.substring(12, 13) !== '0'
                      ? `Class Tomorrow @ ${state.formData.classTime.substring(
                          12,
                        )}`
                      : `Class Tomorrow @ ${state.formData.classTime.substring(
                          11,
                ...
                <Button
                  ...
                  style={{
                    fontSize: '18px',
                    backgroundColor: state.save ? 'grey' : '#711313',
                    color: 'white',
                  }}
            ...
            >
              ...
              <Grid item xs={7}>
                <div style={{ margin: 'auto', width: '93%' }}>
                  <Typography>First Call Notes:</Typography>
                  <TextareaAutosize
                    minRows={5}
                    maxRows={9}
                    style={{
                    ...
                    onBlur={text => {
                      if (state.firstCallMessage !== text.target.value) {
                        updateFirstCallState({
                          type: 'setFirstCallMessage',
                          data: text.target.value.replace(/\n/g, '     '),
                        });
                      }
                    }}
                  />
              ...
              <Grid item xs={12} md={4}>
                <div
                  style={{
                    margin: 'auto',
                    width: '92%',
                    opacity: state.isProcessing ? 0.2 : 1,
                  }}
                >
                  <Typography
                    style={{
              ...
              <Grid item xs={12} md={8}>
                ...
                <div
                  style={{
                    ...
                    width: '98%',
                    margin: 'auto',
                    display: state.formData.division === 2 ? '' : 'none',
                  }}
                ></div>
                <div
                  style={{
                  ...
                  {state.googleApiKey != '' && (
                    <DispatchMap
                      ...
                      users={state.users.filter(user => {
                        const techIds = state.techs.map(tech =>
                          tech.getUserId(),
                        );
      ...
      <Modal open={state.openModal} onClose={resetModal}>
        <ModalAlert
        ...
        {state.modalKey === 'callInfo' && (
          ...
          >
            <Box
              sx={{ width: '100%', position: 'sticky', top: '20px', zIndex: 3 }}
            >
              <Collapse in={state.hasNotification}>
                <Alert
                  action={
            ...
            >
                        ...
                        <TableBody>
                          ...
                          <TableRow>
                            ...
                            <TableCell
                              style={{ width: '65%', textAlign: 'center' }}
                            >
                              {state.currentFCCall.propertyCity}
                            </TableCell>
                          ...
                          <TableRow>
                            ...
                            >{`${state.currentFCCall.jobType}/${state.currentFCCall.subType}`}</TableCell>
                          ...
                          <TableRow>
                            ...
                            <TableCell style={{ textAlign: 'center' }}>
                              {state.currentFCCall.description.length >= 200
                                ? state.currentFCCall.description
                                    .slice(0, 150)
                                    .concat(' ...')
                                : state.currentFCCall.description}
                            </TableCell>
                        ...
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell
                              style={{ width: '65%', textAlign: 'center' }}
                            >
                              {state.selectedCall.getCustName()}
                            </TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell
                            ...
                            <TableCell
                              style={{ width: '65%', textAlign: 'center' }}
                            >
                              {state.selectedCall.getPropertyAddress()}
                            </TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell
                            ...
                            <TableCell
                              style={{ width: '65%', textAlign: 'center' }}
                            >
                              {state.selectedCall.getUserEmail()}
                            </TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell
                            ...
                            <TableCell
                              style={{ width: '65%', textAlign: 'center' }}
                            >
                              {state.selectedCall.getUserPhone()}
                            </TableCell>
                          </TableRow>
                        </TableBody>
                      </Table>
                        ...
                        <TableRow>
                          ...
                          <TableCell>
                            ...
                            <Typography style={{ color: 'red' }}>
                              *Note: Removing all assignees from call will
                              remove this note!
                            </Typography>
                          </TableCell>
                        </TableRow>
              ...
              >
                ...
                >
                  ...
                  {state.assigneeList.length > 0 &&
                    state.assigneeList.map((assignee, index) => {
                      return (
                        <ListItem key={index}>
                          <ListItemIcon
                            onClick={() => {
                              if (!state.isProcessing)
                                handleUpdateAssignTech(assignee.id, 'remove');
                            }}
                          >
                            <RemoveCircleOutlineTwoTone color="primary" />
                          </ListItemIcon>
                  ...
                  <ListItem key="addNew">
                    <ListItemIcon onClick={handleAddTechToggle}>
                      <AddCircleOutlineTwoTone color="secondary" />
                    </ListItemIcon>
                    <ListItemText>
                      <PlainForm
                        schema={[
                          [
                            {
                              ...
                              options: state.techs
                                .filter(
                                  tech =>
                                    (state.formData.availableTechs.includes(
                                      tech,
                                    ) &&
                                      !state.firstCallInUse.includes(
                                        tech.getUserId(),
                                      ) &&
                                      !state.pendingAddInUse.includes(
                                        tech.getUserId(),
                                      )) ||
                                    state.pendingRemoveInUse.includes(
                                      tech.getUserId(),
                                    ),
                                )
                                .sort((a, b) =>
                        ...
                        onChange={callback => {
                          if (callback.tempAssigneeList !== '') {
                            handleUpdateAssignTech(
                              Number(callback.tempAssigneeList),
                              'add',
                            );
        ...
        {state.modalKey === 'editRequest' &&
          state.selectedCall.getPropertyId() !== 0 && (
            <ServiceRequest
        ...
        {state.modalKey === 'fcPreview' && (
          ...
          >
            ...
            <div style={{ opacity: state.isProcessing ? 0.2 : 1 }}>
              <Grid container style={{ verticalAlign: 'middle' }}>
                ...
                <Grid item xs={3}>
                  <Checkbox
                    ...
                    onChange={() =>
                      updateFirstCallState({
                        type: 'setFinalApproval',
                        data: !state.isApproved,
                      })
                    }
              ...
              <div
                style={{ border: 1, borderWidth: '1px', borderStyle: 'solid' }}
              >
                <Grid
                  container
                ...
                >
                  ...
                  <Grid item xs={6} style={{ paddingTop: '15px' }}>
                    <Grid container>
                      <Grid
                        ...
                        style={{ fontWeight: 'bolder', fontSize: '16px' }}
                      >
                        {!state.savedFirstCall.class.isTomorrow
                          ? `No Class Tomorrow`
                          : `Class Tomorrow @ ${state.savedFirstCall.class.start}`}
                      </Grid>
                      {state.savedFirstCall.class.list.length > 0 &&
                        state.savedFirstCall.class.list.map(tech => (
                  ...
                  <Grid item xs={6} style={{ paddingTop: '15px' }}>
                    <Grid container>
                      <Grid
                        ...
                        style={{ fontWeight: 'bolder', fontSize: '16px' }}
                      >
                        {!state.savedFirstCall.meeting.isTomorrow
                          ? `No Meeting Tomorrow`
                          : `Meeting Tomorrow @ ${state.savedFirstCall.meeting.start}`}
                      </Grid>
                      {state.savedFirstCall.meeting.list.length > 0 &&
                        state.savedFirstCall.meeting.list.map(tech => (
                  ...
                  <Grid item xs={6} style={{ paddingTop: '15px' }}>
                    <Grid container>
                      ...
                      {state.savedFirstCall.manualOff.length > 0 &&
                        state.savedFirstCall.manualOff.map(tech => (
                  ...
                  <Grid item xs={6} style={{ paddingTop: '15px' }}>
                    <Grid container>
                      ...
                      {state.savedFirstCall.scheduledOff.length > 0 &&
                        state.savedFirstCall.scheduledOff.map(tech => (
                  ...
                  {state.savedFirstCall.calls.length > 0 &&
                    state.savedFirstCall.calls.map(call => (
