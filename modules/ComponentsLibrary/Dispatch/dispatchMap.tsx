      ...
      const markers = props.users.map(async user => {
        ...
        if (user.getGeolocationLng() && user.getGeolocationLat()) {
          return (
            <Marker
              position={{
                lat: user.getGeolocationLat(),
                lng: user.getGeolocationLng(),
              }}
              icon={icon}
              label={{
          ...
          // If no address or zip, notify user
    ...
    const markers = props.techs.map(async tech => {
      ...
      if (tech.getGeolocationLng() && tech.getGeolocationLat()) {
        return (
          <Marker
            position={{
              lat: tech.getGeolocationLat(),
              lng: tech.getGeolocationLng(),
            }}
            icon={icon}
            label={{
              text: tech.getTechname(),
              fontWeight: 'bold',
              fontSize: '15px',
            }}
        ...
        const geocode = new google.maps.Geocoder();
        try {
          const results = await geocode.geocode({
            address: tech.getPropertyCity(),
          });
          return (
            <Marker
              position={{
      ...
      if (call.getGeolocationLng() && call.getGeolocationLat()) {
        return (
          <Marker
            ...
            onClick={() =>
              props.handleMapClick(call, false, new DispatchableTech())
            }
        ...
        try {
          const results = await geocode.geocode({
            address: call.getPropertyAddress(),
          });
          const lat = results.results[0].geometry.location.lat();
          const lng = results.results[0].geometry.location.lng();
          props.handleGeocodeUpdate('property', call.getPropertyId(), {
            lat,
            lng,
          });
          return (
