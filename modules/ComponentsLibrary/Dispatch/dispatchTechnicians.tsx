...
export const DispatchTechs: FC<props> = props => {
    ...
    (techs: DispatchableTech[]) => {
      ...
      if (isFirstCall) {
        const available = sorted.filter(
          tech => !firstCallInUse.includes(tech.getUserId()),
        );
        const assigned = sorted.filter(tech =>
          firstCallInUse.includes(tech.getUserId()),
        );
  ...
  return (
    <div>
      {loading && <InfoTable data={makeFakeRows(5, 3)} loading />}
      {!loading && (
        <TableContainer
                ...
                <TableBody ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  {techs.length > 0 &&
                    sortedTechnicians.map((tech, index) => {
                      const timeOnHours = Math.floor(
                        differenceInMinutes(
                          new Date(),
                          parseISO(tech.getActivityDate()),
                        ) / 60,
                      );
                      const timeOnMinutes =
                        differenceInMinutes(
                      ...
                      const hoursWorked = Math.floor(
                        tech.getHoursWorked() / 3600,
                      );
                      const minutesWorked = Math.floor(
                        (tech.getHoursWorked() - hoursWorked * 3600) / 60,
                      );
                      const techLatitude = tech.getGeolocationLat()
                        ? tech.getGeolocationLat()
                        : 0;
                      const techLongitude = tech.getGeolocationLng()
                        ? tech.getGeolocationLng()
                        : 0;
                      let timeoffData: {
                      ...
                      if (props.offTechs && props.offTechs.length) {
                        timeoffData = props.offTechs.find(
                          req => req.tech.getUserId() === tech.getUserId(),
                        )!;
                      }
                      return (
                        <Draggable
                        ...
                        >
                          {(dragProvided, snapshot) => (
                            <TableRow
                              ...
                              style={{
                                backgroundColor:
                                  snapshot.draggingOver === 'dismissTech' ||
                                  snapshot.draggingOver === 'onCallDroppable'
                                    ? '#711313'
                                ...
                                textAlign: 'center',
                                opacity: firstCallInUse.includes(
                                  tech.getUserId(),
                                )
                                  ? 0.4
                                  : 1,
                            ...
                            >
                              <TableCell
                                ...
                                style={{
                                  color:
                                    snapshot.draggingOver &&
                                    snapshot.draggingOver !== 'techDroppable'
                                      ? 'white'
                                      : 'black',
                                }}
                              >
                                {timeoffData && (
                                  <Tooltip
                                    title={
                                      <h2 style={{}}>{`Time Off: ${format(
                                        parseISO(timeoffData.start),
                                        'h:mm aaa',
                                      )} - ${format(
                                        parseISO(timeoffData.end),
                                        'h:mm aaa',
                                      )}`}</h2>
                                    }
                                    placement={'top'}
                  ...
                  >
                    <TableCell
                      style={{
                        ...
                        fontSize: '16px',
                        fontWeight: 'bold',
                        textAlign: !props.isFirstCall ? 'right' : 'center',
                        width: '60%',
                      }}
                    >
                      {!props.isFirstCall
                        ? `Dismiss Technician`
                        : 'Set Technician As Off Tomorrow'}
                    </TableCell>
                    <TableCell
                      style={{
                        color: snapshot.isDraggingOver ? 'white' : '#711313',
                        alignItems: 'left',
                        display: props.isFirstCall ? 'none' : '',
                      }}
                    >
