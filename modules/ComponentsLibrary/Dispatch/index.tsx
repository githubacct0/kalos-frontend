...
export const DispatchDashboard: React.FC<Props> = function DispatchDashboard({
  ...
  const handleDismissTech = useCallback(
    async (
      overrideId: number = 0,
      suppressSlack: boolean = false,
      skipRefresh: boolean = false,
    ) => {
      setProcessing(true);
      const actLog = new ActivityLog();
      const errorMessage =
        '- Unable to Create Activity Log and Service Rendered for Dismissal.';
      const techId =
        overrideId > 0 ? overrideId : state.selectedTech.getUserId();
      ...
      try {
        ...
        if (!disableSlack && !suppressSlack)
          SlackClientService.DirectMessageUser(
            testUserId ? 103939 : techId,
            `Go Home, ${state.selectedTech.getTechname()}`,
            loggedUserId,
          );
        if (!suppressSlack)
          handleNotification(
            `${state.selectedTech.getTechname()} Successfully Dismissed`,
            'success',
            true,
          );
      } catch (err) {
  ...
  const handleUndismissTech = useCallback(
    async (
      tech: DispatchableTech,
      suppressSlack = false,
      skipRefresh = false,
    ) => {
      ...
      const actLog = new ActivityLog();
      const errorMessage =
        '- Unable to Create Activity Log and Service Rendered for Undismissal.';
      actLog.setUserId(loggedUserId);
      actLog.setPropertyId(19139);
      ...
      try {
        ...
        if (!disableSlack && !suppressSlack)
          SlackClientService.DirectMessageUser(
            testUserId ? 103939 : tech.getUserId(),
            `False Alarm, ${tech.getTechname()}!  I need you back on the schedule!`,
            loggedUserId,
          );
        if (!suppressSlack)
          handleNotification(
            `${tech.getTechname()} Successfully Undismissed!`,
            'success',
            true,
          );
      } catch (err) {
  ...
  const getTechnicians = useCallback(async () => {
    ...
    try {
      ...
      const techs = await DispatchClientService.GetDispatchableTechnicians(
        tech,
      );
      const techList = techs.getResultsList().map(tech => tech.getUserId());
      const offTechs = offEmployees.timeOff.filter(tech =>
        techList.includes(tech.getUserId()),
      );
      ...
      offTechs.forEach(req => {
        const tech = techs
          .getResultsList()
          .find(tech => tech.getUserId() === req.getUserId())!;
        requestOffData = requestOffData.concat({
          tech: tech,
          start: req.getTimeStarted(),
          end: req.getTimeFinished(),
        });
        if (
          format(new Date(), `yyyy-MM-dd HH:mm:ss`) >= req.getTimeStarted() &&
          format(new Date(), `yyyy-MM-dd HH:mm:ss`) <= req.getTimeFinished()
        ) {
      ...
      const availableTechs = techs
        .getResultsList()
        .filter(
          tech =>
            tech.getActivity() != 'Dismissed' &&
            !requestedOffTechs.includes(tech),
        );
      const dismissedTechs = techs
        .getResultsList()
        .filter(
          tech =>
            tech.getActivity() === 'Dismissed' &&
      ...
      return {
        ...
        dismissed: dismissedTechs,
        off: requestedOffTechs,
        offData: requestOffData,
      };
    } catch (err) {
      handleNotification(errorMessage, 'error', true);
      console.error(`An error occurred while getting Dispatch Techs: ${err}`);
      return { available: [], dismissed: [], off: [], offData: [] };
    }
  }, [
    state.formData.departmentIds,
    state.defaultDepartmentIds,
    handleNotification,
    checkErrors,
    getTimeOffEmployees,
    handleDismissTech,
  ...
  const getCalls = useCallback(async () => {
    ...
    call.setDateRangeList([
      '>=',
      state.formData.dateStart,
      '<=',
      state.formData.dateEnd,
    ]);
    ...
    try {
      ...
      const filteredCalls = callResults.filter(
        call =>
          `${call.getDateStarted()} ${call.getTimeStarted()}` >=
            `${state.formData.dateStart} ${state.formData.timeStart.substring(
              11,
            )}` &&
          `${call.getDateEnded()} ${call.getTimeEnded()}` <=
            `${state.formData.dateEnd} ${state.formData.timeEnd.substring(11)}`,
    ...
    state.formData.dateStart,
    state.formData.timeStart,
    state.formData.dateEnd,
    state.formData.timeEnd,
  ...
  const getDepartments = useCallback(async () => {
    ...
    try {
      ...
      let displayedDepartments = departments
        .getResultsList()
      ...
      return {
        ...
        defaultValues: displayedDepartments.map(dep => dep.getId()),
      };
    } catch (err) {
      handleNotification(errorMessage, 'error', true);
      // console.error(
  ...
  const getJobTypes = useCallback(async () => {
    ...
    try {
      ...
      const displayedJobTypes = jobTypes
        .getResultsList()
        .filter(
          jobType => !state.notIncludedJobTypes.includes(jobType.getId()),
        );
      checkErrors(errorMessage);
  ...
  useEffect(() => {
    ...
    state.refreshTechnicians,
    state.initialTechLoad,
  ]);
  ...
  const handleChange = async (formData: FormData) => {
    ...
    if (
      state.formData.departmentIds.length != formData.departmentIds.length ||
      !state.formData.departmentIds.every(
        (val, index) => val === formData.departmentIds[index],
      )
    ) {
      ...
      state.departmentList
        ...
        .forEach(dep =>
          !formData.divisionMulti.includes(dep.getSectorGroup())
            ? formData.divisionMulti.push(dep.getSectorGroup())
            : formData.divisionMulti,
        );
    }
  ...
  const handleAssignTech = async () => {
    ...
    const ids =
      state.selectedCall.getLogTechnicianAssigned() != '0' &&
      state.selectedCall.getLogTechnicianAssigned() != ''
    ...
    try {
      ...
      if (!disableSlack)
        SlackClientService.Dispatch(
          state.selectedCall.getId(),
          testUserId ? 103939 : state.selectedTech.getUserId(),
          loggedUserId,
        );
      handleNotification(
        `${state.selectedTech.getTechname()} Successfully Assigned to Call ${state.selectedCall.getId()}`,
        'success',
        true,
      );
    } catch (err) {
  ...
  const handleUnassignTech = async (id: number) => {
    ...
    event.setId(state.selectedCall.getId());
    const idArray = state.assigneeList
      .map(assignee => assignee.id)
      .filter(techId => techId !== id)
      .map(String);
    ...
    if (!error) {
      ...
      handleNotification(
        `${tech.getTechname()} Successfully Unassigned from Call ${state.selectedCall.getId()}`,
        'success',
        true,
      );
    }
      ...
      if (url.searchParams.get('code')) {
        ...
        const token = await axios.get(
          `https://slack.com/api/oauth.access?client_id=${slackClientId}&client_secret=${secretKey.getApiKey()}&code=${url.searchParams.get(
            'code',
          )}&redirect_uri=https://app.kalosflorida.com/index.cfm?action=admin:dispatch.dispatch_mode`,
        );
  ...
  const setInitialValues = useCallback(async () => {
    ...
    const defaultSectors = getSectorGroups(
      results[0].departments.filter(dep =>
        results[0].defaultValues.includes(dep.getId()),
      ),
    );
    updateDispatchState({
  ...
  const SCHEMA_PRINT: Schema<FormData> = [
    [
      ...
      },
      {
        name: 'divisionMulti',
        label: 'Sectors(s)',
        options: ['Residential', 'Commercial Light', 'Commercial Heavy'].map(
          (item, index) => ({
            key: item,
            label: item,
            value: index + 1,
  ...
  return (
    <PageWrapper userID={loggedUserId}>
      ...
      {state.isLoadingMap &&
        state.isLoadingTech &&
        state.isLoadingCall &&
        state.checkUser && <Loader backgroundColor={'black'} opacity={0.5} />}
      <SectionBar
        title="Dispatch"
        ...
        <div>
              ...
              <DragDropContext
                onDragEnd={async callback => {
                  const tech = state.techs.find(
                    i => i.getUserId() === Number(callback.draggableId),
                  );
                  let call = new DispatchCall();
                  let modalKey = 'Dismiss';
                  if (
                    callback.destination &&
                    callback.destination.droppableId != 'dismissTech'
                  ) {
                    call = state.calls.find(
                      i =>
                        i.getId() === Number(callback.destination!.droppableId),
                    )!;
                    modalKey = 'Assign';
                  }
                  ...
                  <Grid item md={6} xs={12}>
                    <div
                      style={{
                        ...
                        margin: 'auto',
                        textAlign: 'center',
                        display: state.dismissedTechs.length ? '' : 'none',
                      }}
                    >
                      <Button
                        style={{
                        ...
                        onClick={handleUndismissButtonClick}
                      >
          ...
          >
            ...
            {state.modalKey === 'Dismiss' && (
              ...
              >
                <h3>
                  Send {state.selectedTech!.getTechname()} Home for the Day?
                </h3>
              </Confirm>
            )}
            {state.modalKey === 'Assign' && (
              <Confirm
                ...
                submitLabel={
                  state.isProcessing ? 'Saving...' : 'Assign Tech to Call'
                }
                        ...
                        <Table>
                          ...
                          <TableBody>
                            <TableRow>
                              ...
                              >
                                {state.selectedTech.getTechname()}
                              </TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell
                              ...
                              <TableCell style={{ textAlign: 'center' }}>
                                {state.selectedTech.getPropertyCity()}
                              </TableCell>
                            </TableRow>
                          </TableBody>
                        </Table>
                        ...
                        <Table>
                          ...
                          <TableBody>
                            <TableRow>
                              ...
                              >
                                {state.selectedCall.getPropertyCity()}
                              </TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell
                            ...
                            <TableRow>
                              ...
                              >{`${state.selectedCall.getJobType()}/${state.selectedCall.getJobSubtype()}`}</TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell
                                style={{ fontWeight: 'bold', fontSize: '15px' }}
                              ...
                              <TableCell>
                                {state.selectedCall.getDescription().length >=
                                200
                                  ? state.selectedCall
                                      .getDescription()
                                      .slice(0, 150)
                                      .concat(' ...')
                                  : state.selectedCall.getDescription()}
                              </TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell
                              ...
                              <TableCell style={{ textAlign: 'center' }}>
                                {state.selectedCall.getAssigned() != '0'
                                  ? state.selectedCall.getAssigned()
                                  : 'Unassigned'}
                              </TableCell>
                            </TableRow>
                          </TableBody>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        ...
                        <TableCell style={{ textAlign: 'center' }}>
                          {state.selectedCall.getDescription().length >= 200
                            ? state.selectedCall
                                .getDescription()
                                .slice(0, 150)
                                .concat(' ...')
                      ...
                      <TableRow>
                        ...
                        <TableCell style={{ textAlign: 'center' }}>
                          {state.selectedCall.getLogNotes()}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell
            ...
            {state.modalKey === 'callInfo' && (
              ...
              >
                ...
                <Grid
                  container
                  spacing={1}
                  style={{ width: window.innerWidth * 0.85 }}
                >
                              ...
                              <ListItemIcon
                                onClick={() => {
                                  if (!state.isProcessing)
                                    handleUnassignTech(assignee.id);
                                }}
                              >
